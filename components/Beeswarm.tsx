...
export default function circleChart({ svgEl, rootSelection, scale, data = [] }: CircleChartParams) {
  ...
  tooltips
    ...
    .text((d: Datum) => {
      ...
      const MAX_LENGTH = 60
      return d.title.length > MAX_LENGTH ? `${d.title.slice(0, MAX_LENGTH)}…` : d.title
    })
    .attr('class', 'tooltipText')
    .attr('text-anchor', 'start')
    .attr('transform', (d: Datum) => `translate(${TOOLTIP_TEXT_PADDING}, -${d.radius + 35})`)
  ...
  tooltips
    ...
    .attr('transform', (d: Datum) => `translate(${TOOLTIP_TEXT_PADDING}, -${d.radius + 20})`)
