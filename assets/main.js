	  ...
	  jsonp(LINKS_URL, function (links) {
	    ...
	    for (var j = tags.length - 1; j >= 0; j--) {
	      ...
	      circleChart({
	        ...
	        radius: function (d) {
	          return d.t.indexOf("top") === -1 ? radius : radius * 1.8;
	        },
	        el: linksSvg,
	        groupClass: tag,
	        timeProp: "dt",
...
/***/ function(module, exports, __webpack_require__) {
	...
	var d3 = __webpack_require__(4);
	var forceChart = __webpack_require__(5);
	...
	module.exports = function circleChart(config) {
	  ...
	  var bubbleChart = forceChart().size([config.width, 300]) // FIXME
	  .padding(1).x(x.fromDateString({ propName: config.timeProp })).xStart(x.fromDateString({ propName: config.timeProp })).y(yBaseline).yStart(function (d) {
	    return yBaseline + 10 - 20 * Math.random();
	  }).r(radius).xGravity(8) // make the x-position more accurate
	  .yGravity(5); // ...and the y-position more flexible

	  var groups = config.el.append("g").call(bubbleChart, config.data).attr("class", "bubbles").selectAll(".node");

	  var links = groups.append("a").attr("xlink:href", function (d) {
	    return d[config.urlProp];
	  });

	  links.append("circle").attr("r", function (d) {
	    return d.r0;
	  }).attr("fill", "slategrey");

	  links.append("line").attr("x1", 1).attr("x2", 1).attr("y1", function (d) {
	    var radius = parseFloat(d3.select(this.parentElement.firstChild).attr("r"));
	    return radius + 3;
	  }).attr("y2", function () {
	    return parseFloat(d3.select(this).attr("y1")) + 40;
	  });
	  ...
	  links.append("text").attr("class", "date").text(function (d) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = function () {
	  ...
	  var force = d3.layout.force().charge(0).gravity(0);
	  ...
	  function chart(selection, nodes) {
	    ...
	    var gNodes = selection.selectAll(".node").data(nodes).enter().append("g").attr("class", "node").call(draggable ? force.drag : function () {});

	    force.size([width, height]).nodes(nodes).on("tick", tick).start();

	    function tick(e) {
	      gNodes.each(gravity(e.alpha * 0.1)).each(collide(0.5)).attr("transform", function (d) {
